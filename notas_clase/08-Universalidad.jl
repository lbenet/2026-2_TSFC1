...
# exhibe una cascada de bifurcaciones de doblamiento de periodo.
# Esto lo vimos, inicialmente de forma analítica y sólo hasta el periodo 4,
# y después de manera numérica, cubriendo el intervalo $c\in[-1/4, 2]$.
# Sin embargo, la resolución numérica eventualmente se vuelve insuficiente,
# en el sentido de que es difícil de interpretar por
# lo complicado que es visualizar las cosas y entender qué pasa.

# La pregunta que buscaremos resolver es si uno *puede* llegar a
# *periodo infinito* para un valor finito $c_\infty\in [-1/4, 2]$,
# de hecho menor a $2$, para el mapeo cuadrático, usando herramientas
# numéricas y algunas analíticas.
# Periodo infinito en este caso equivale a que no hay periodicidad,
# y entonces uno se pregunta si hay una noción de estabilidad para
# órbitas genéricas en este régimen de órbitas aperiódicas. Como
# veremos, no hay tal estabilidad sino inestabilidad (excepto por
# un conjunto de medida cero), y la propia dinámica refleja lo que
# llamaremos *caos*.
# Abordaremos estas cuestiones, y además abordaremos la
# *universalidad* del escenario.
...
# evaluaremos si la evolución de la separación de dos condiciones iniciales,
# inicialmente infinitesimalmente cercanas, exhibe una dependencia *exponencial* en el
# tiempo. Si éste es el caso entonces diremos que hay sensibilidad a las condiciones
# iniciales, es decir, caos.
...
# $d_n= d_0 \exp(\lambda n)$, con $d_0 = \epsilon$. De aquí obtenemos
# ```math
# \begin{equation}
...
# El que no haya caos debe interpretarse como que el movimiento es periódico
# (o quizás cuasi-periódico) en algún sentido; en este caso, las dos condiciones
# iniciales se acercarán, y entonces a partir de conocer la evolución de una podemos
# conocer la evolución de la otra.
#
# En cambio, que haya
# caos ($\lambda(x_0)>0$) significa que a partir de conocer la evolución de una
# condición inicial no podemos decir mucho sobre la evolución de la otra. En este
# caso, resulta que hay regiones del espacio $x$ cuya
# dinámica es no periódica, aunque pueden existir condiciones iniciales que
# tengan órbitas periódicas.

# Observaciones:
#
# 1. El exponente de Lyapunov, estrictamente hablando, depende de la condición inicial $x_0.$
#
# 2. En la definición del exponente de Lyapunov se require la evaluación de **dos** límites, uno que involucra al tiempo ($n\to\infty$), y otro que representa la distancia entre las condiciones iniciales ($\epsilon\to 0$).
#
# 3. La definición del exponente de Lyapunov es sutil, ya que en muchas ocasiones *sabemos* que el rango de $f(x)$ es acotado cuando $x$ está en cierto dominio (como en el mapeo cuadrático), lo que entonces podría llevar erróneamente a concluir que $\lambda(x_0)=0$. La sutileza está evidentemente en el órden de los límites: los límites, en general, no conmutan.
#
# 4. Las unidades del exponente de Lyapunov corresponden al inverso del tiempo, es decir, $\lambda^{-1}$ describe una escala relevante del tiempo en que condiciones iniciales se separan una distancia del orden de $e$ ($\exp(1)$).

# Si el mapeo $f(x)$ es suficientemente suave, entonces podemos escribir $(2)$ como
# ```math
# \begin{equation}
# \lambda(x_0) = \lim_{n\to\infty} \frac{1}{n}\log\Big|
# \frac{{\rm d} }{{\rm d}x}f^{n}(x_0)\Big|,
# \tag{3}
# \end{equation}
# ```
# donde literalmente hemos usado la definición de la derivada de la función
# $f^{n}$, el $n$-ésimo iterado de $f(x)$.
#
# Para mapeos en una dimensión entonces se cumple
# ```math
# \begin{equation*}
# \frac{{\rm d}f^n}{{\rm d}x}(x_0) = f'(x_0) f'(x_1)\dots f'(x_{n-1}) = \prod_{i=0}^{n-1} f'(x_i),
# \end{equation*}
# ```
# donde $x_i=f^i(x_0)$, es decir, $x_i$ es el i-ésimo iterado de $x_0$. De aquí entonces obtenemos
# ```math
# \begin{equation}
...
# Las siguiente función la definimos en la clase anterior, y la
...
ff = diag_bifurc(Q, crange, 10_000, 256);
...
# Ahora graficamos, primero, el diagrama de bifurcaciones, y después el
# resultado obtenido para el exponente de Lyapunov.

begin
	p1 = scatter(cc, ff, markersize=0.05, markerstrokestyle=0,
...
begin
    ...
    xlims!(p2, -0.25, 2.0)
    ylims!(p2, -1.5, 1.0)
    plot!(p2, [-0.25, 2.0], zero, color=:red)
    # savefig("diag_lyap1.png");
end

# Hay varias observaciones interesantes que se desprenden de estas figuras:
#
# - Cada vez que ocurre una bifurcación de doblamiento de periodo, el exponente de Lyapunov es cero. Esto es una simple consecuencia de que en el punto de bifurcación (de doblamiento de periodo), la derivada es -1; ver ecuación $(4)$.
#
# - Cada vez que ocurre un ciclo superestable, es decir, que $x=0$ es parte de la órbita periódica (por ejemplo, de periodo $2^n$), el exponente de Lyapunov diverge a $-\infty$. Esto es consecuencia de que en este caso, la derivada (en $x=0$) se anula.
#
# - Al aumentar $c$, en algún momento el exponente de Lyapunov se hace positivo. Esto es, todos los iterados de las órbitas típicas son localmente inestables.
#

# A partir de los cálculos que hemos hecho, podemos obtener una aproximación del valor de $c$ en que el exponente de Lyapunov se vuelve positivo por primera vez (al aumentar $c$).
...
# éste es el caso más alejado de -1 o 1, que son los puntos donde se pueden dar  posibles bifurcaciones (estabilidad neutral).
...
# $c$ es de orden 8... Aquí, simplemente usaré una buena
...
# $Q_{c_2}^4(x)$ que, localmente, se *parece* a $Q_{c_0}(x)$.
...
# periodo 8. En este caso, usamos
# $c_3 = 1.38154748\dots$.

begin
	c_3 = 1.3815474844320617

	local xrange = -1:1/128:1
...
# Vale la pena además notar que la distancia al punto fijo (que cruza
...
# \end{equation}
# ```
# es decir, la mitad de los iterados del mapeo.
...
# La otra constante de Feigenbaum, $\delta$, se obtiene estudiando las
# propiedades de la ecuación linearizada en $c$.
# La aproximación numérica de ambas constantes es parte de la Tarea 3.
