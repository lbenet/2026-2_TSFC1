...
# Aquí, $\mu$ representa uno o varios parámetros que uno puede variar,
# y $x_n$ representa
# el estado del sistema al "tiempo" (discreto) $n$.

# La idea de introducir mapeos es evitar las complicaciones que surgen al resolver ecuaciones diferenciales, lo que involucraría un
...
# Por otra parte, para $F(x) = x^2+1$ ($x\in\mathbb{R}$), la definición de punto fijo
...
# órbitas divergen a $+\infty$. (Otra manera de formular esto último, menos rigurosa, es que el único punto fijo *atractivo* es $+\infty$; es una formulación menos rigurosa ya que $\infty\not\in\mathbb{R}$.)

# Numéricamente, verificar si cierta condición inicial es un punto fijo de un mapeo es
# en principio sencillo, aunque también delicado. Concretamente, para $G(x)$ definido arriba debemos
...
# \end{equation}
# ```
# Claramente, de la ecuación que define la órbita periódica, concluimos que un punto de periodo $n$ del mapeo $F$ es un punto fijo (de periodo 1) del mapeo definido por la función $F^n$.
# Además, de la definición, la ecuación tiene *al menos* $n$ soluciones distintas.
...
# El análisis gráfico es útil pero definitivamente no es riguroso, y esencialmente sólo funciona en dimensión 1. Esto se puede deber a cuestiones de precisión numérica o simplemente a que el número de iteraciones es finito y podría ser insuficiente.
...
# convergen a 0, que es el *único* punto fijo;
# en algún sentido, para $0<\alpha<1$ el punto fijo los *atrae*.

begin
    x12 = itera_mapeo(x->f(x, 0.6), 0.8, 20)
    x22 = itera_mapeo(x->f(x, 0.6), -0.8, 20)
    x12[end], x22[end]
end
...
# ```math
# \begin{equation}
# x_n = \alpha x_{n-1} = \dots = x_0 \alpha^n.
# \end{equation}
# ```
...
begin
	plot(-1:1/32:1, f1,
	    ...
	    color=:blue, framestyle=:zerolines)
...
# En este apartado analizaremos la dinámica del mapeo \eqref{eq:mapeoBern}, ilustrando las
...
# Usando la definición, Eq. \eqref{eq:mapeoBern}, es fácil obtener
# ```math
# \begin{equation}
# \sigma^2(x) =
# \begin{cases}
...
# A partir de $\sigma^2(x)=x$ y usando \eqref{eq:periodo2}, obtenemos los puntos
...
# Uno puede generalizar fácilmente la Eq. \eqref{eq:periodo2} a cualquier periodo
...
begin
	y0 = 1/3
	println("n = ", 0, "   x_n = ", bitstring(y0))
	for its = 1:55
	    y0 = σ(y0)
	    (its < 10 || its > 45) && println("n = ", its, "   x_n = ", bitstring(y0))
	end
end
...
# en la cola. Esto, literalmente borra un bit de precisión por iteración, hasta que todos los bits
...
# \end{equation}
# ```
# donde, dado que estamos usando base 2, tenemos que $a_k$ es 0 o 1. Por ejemplo, si
# $x_0 < 1/2$ tendremos que $a_1=0$, y si $x_0 \ge 1/2$ entonces $a_1=1$.

# Ahora explotaremos la representación binaria para entender la
# dinámica; ésta, para la condición inicial, corresponde a
# $x_0 = \sum_{k=1} a_k 2^{-k}$.
# Como explicamos anteriormente, el mapeo \eqref{eq:mapeoBern} equivale a
...
# Esta ecuación nos permite entender la dinámica de manera muy precisa,
...
# Estas caracterísiticas, como veremos más adelante, están íntimamente
# relacionadas con el conceto de caos.
