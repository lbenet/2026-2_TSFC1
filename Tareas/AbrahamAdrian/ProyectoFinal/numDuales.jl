...
module numDuales
    ...
    import Base: +, -, *, /, sin, cos, tan, ^, sqrt, exp, log, ≈, ==, mod
    ...
    *(u::Dual, w::Dual) = Dual(u.fun * w.fun, (u.fun * w.der) + (w.fun * u.der))
    ...
    ==(u::Dual, v::Dual) = u.fun == v.fun && u.der == v.der
    # mod(x, c): la derivada de mod respecto a x es 1 en casi todo punto (salvo discontinuidades).
    mod(u::Dual, c::Real) = Dual(mod(u.fun, c), u.der)
