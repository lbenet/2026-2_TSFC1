...
function Dual(c::T) where {T<:Real}
    return Dual(c,zero(c))
end
...
isapprox(x::Dual, y::Dual; kargs...) = isapprox(x.fun, y.fun;kargs...) && isapprox(x.der, y.der; kargs...)
...
@testset "trigonometrics" begin
    ...
    @test sin(dr) ≈ -sqrt((1-cos(2*dr))/(2)) atol=0.000001  
end
